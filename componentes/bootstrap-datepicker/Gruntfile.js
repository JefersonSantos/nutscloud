module.exports = function(grunt){
    ...
    grunt.initConfig({
        ...
        less: {
            ...
            standalone_bs2: {
                ...
                src: 'construtor/build_standalone.less',
                dest: 'bibliotecas/css/<%= pkg.name %>.standalone.css'
            },
            standalone_bs3: {
                options: {
                    sourceMapURL: '<%= pkg.name %>3.standalone.css.map'
                },
                src: 'construtor/build_standalone3.less',
                dest: 'bibliotecas/css/<%= pkg.name %>3.standalone.css'
            },
            main_bs2: {
                options: {
                    sourceMapURL: '<%= pkg.name %>.css.map'
                },
                src: 'construtor/construtor.less',
            ...
            main_bs3: {
                ...
                src: 'construtor/build3.less',
                dest: 'bibliotecas/css/<%= pkg.name %>3.css'
            }
        },
        usebanner: {
            options: {
            ...
            css: 'bibliotecas/css/*.css',
            js: 'bibliotecas/js/**/*.js'
        },
        cssmin: {
            options: {
                compatibility: 'ie8',
            ...
            main: {
                files: {
                    'dist/css/<%= pkg.name %>.min.css': 'bibliotecas/css/<%= pkg.name %>.css',
                    'dist/css/<%= pkg.name %>3.min.css': 'bibliotecas/css/<%= pkg.name %>3.css'
                }
            },
            standalone: {
                files: {
                    'dist/css/<%= pkg.name %>.standalone.min.css': 'bibliotecas/css/<%= pkg.name %>.standalone.css',
                    'dist/css/<%= pkg.name %>3.standalone.min.css': 'bibliotecas/css/<%= pkg.name %>3.standalone.css'
                }
            }
        },
        csslint: {
            ...
            dist: [
                'bibliotecas/css/bootstrap-datepicker.css',
                'bibliotecas/css/bootstrap-datepicker3.css',
                'bibliotecas/css/bootstrap-datepicker.standalone.css',
                'bibliotecas/css/bootstrap-datepicker3.standalone.css'
            ]
        },
        compress: {
            main: {
                options: {
                    archive: '<%= pkg.name %>-<%= pkg.version %>-bibliotecas.zip',
                    mode: 'zip',
                    level: 9,
                    pretty: true
                },
    ...
    grunt.registerTask('bibliotecas-js', ['concat', 'uglify:main', 'uglify:locales', 'usebanner:js']);

    // CSS distribution task.
    grunt.registerTask('less-compile', 'less');
    grunt.registerTask('bibliotecas-css', ['less-compile', 'cssmin:main', 'cssmin:standalone', 'usebanner:css']);

    // Full distribution task.
    grunt.registerTask('dist', ['clean:bibliotecas', 'bibliotecas-js', 'bibliotecas-css']);

    // Code check tasks.
    grunt.registerTask('lint-js', 'Lint all js files with jshint and jscs', ['jshint', 'jscs']);
    grunt.registerTask('lint-css', 'Lint all css files', ['bibliotecas-css', 'csslint:bibliotecas']);
